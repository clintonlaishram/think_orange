...
//
// ⚠️ 13-08-2026: client preview request (Clinton) — every legal page now
// shows its real hero, then the shared <ComingSoon /> (the same component
// every other non-Home/About page uses), regardless of whether `sections`
// is written. This replaces the bespoke `PendingLegal` copy below for
// visual consistency; `PendingLegal` and the full section renderer are kept
// commented out, not deleted, so this reverts cleanly once real, CA-reviewed
// legal content is ready to publish — see ServiceLeaf.jsx's matching note.
...
// reviewed... an unreviewed privacy policy on a site handling PAN and
// Aadhaar-linked verification is a liability." Same honesty discipline as
// ServiceLeaf's PendingLeaf — no fake sections, no invented TOC, still
// fully reachable and still gives a real way to ask a question in the
// meantime. Superseded by the shared <ComingSoon /> above (13-08-2026);
// left here, commented out, so the original per-page copy is easy to
// restore once `sections` starts landing for real.
